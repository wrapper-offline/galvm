package anifire.component
{
	...
	public class ActionSequence
	{
		...
		public function getNextSubActionStartFrame(param1:Number) : Number
		{
			...
			if(Boolean(this._sequence) && this._sequence.length > 0)
			{
				++this._index;
				...
				_loc2_ = Number(this._sequence[this._index]);
		...
		public function shouldChangeSubAction(param1:Number) : Boolean
		{
			...
			if(Boolean(this._sequence) && this.getSubActionIndex(param1) != this._sequence[this._index])
			{
			}
			return false;
		}
		...
		private function resetSequenceIndex() : void
		{
			...
			var _loc2_:Number = NaN;
			var _loc3_:Number = NaN;
			if(Boolean(this._sequence) && Boolean(this._sequence.length > 0) && Boolean(this._subActionDuration))
			{
		...
		public function behaviourToClipFrame(param1:Number) : Number
		{
			...
			if(Boolean(this._sequence) && this._sequence.length > 0)
			{
				...
				if(this._sequenceIndex)
				{
					...
					_loc2_ = Number(this._sequenceIndex.getValueByKey(String(param1)).frameOffset);
				}
				this._index = this.getSequenceIndex(param1);
				_loc3_ = Number(this._sequence[this._index]);
				if(this._subActionStartFrame)
				{
