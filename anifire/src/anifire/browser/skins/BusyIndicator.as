package anifire.browser.skins
{
	...
	[Style(name="symbolColor", inherit="yes", type="uint")]
	public class BusyIndicator extends UIComponent
	{
		...
		private function startRotation() : void
		{
			...
			{
				_loc1_ = Number(getStyle("rotationInterval"));
				if(isNaN(_loc1_))
				{
		...
		{
			return Boolean(this.rotationTimer) && this.rotationTimer.running;
		}
		...
		{
			if(Boolean(designLayer) && !designLayer.effectiveVisibility)
			{
