package anifire.managers
{
	...
	/**
	 * The AmplitudeAnalyticsManager singleton class handles the logging of
	 * user analytics.<br/>
	 * <br/>
	 * Analytic logs are sent to the application host, which handles them at
	 * its own discretion. On GoAnimate, these analytics would be tracked via
	 * Amplitude, however this behavior is not present in community
	 * recreations.
	 */
	public class AmplitudeAnalyticsManager
	{
		...
		public function incrementUserProperty(propName:String, incrementBy:int) : void
		{
			if (ExternalInterface.available) {
				ExternalInterface.call("amplitudeIncrementUserProperty", propName, incrementBy);
			}
		}

		public function log(eventName:String, extraData:Object = null, idfk:Object = null) : void
		{
			if (ExternalInterface.available) {
				ExternalInterface.call("logAmplitudeEvent", eventName, extraData, idfk);
			}
		}
	}
}
